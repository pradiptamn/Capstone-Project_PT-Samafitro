  ...
  <div class="w-full max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
    <div class="flex flex-col lg:flex-row">
      ...
      <div class="lg:w-1/2 p-8 md:p-12 bg-gradient-to-br from-gray-900 to-gray-700">
        <a href="{{ route('login') }}" class="text-white text-base hover:text-gray-300">
          &#8592; Kembali
        </a>

        <div class="text-white mt-8">
          <h2 class="text-2xl md:text-3xl font-bold mb-3">Reset Password</h2>
          <p class="text-gray-300 mb-8">Masukkan email yang terdaftar untuk mencari akun Anda.</p>

          @if (session('status'))
            <div class="mb-4 p-3 bg-green-600 text-white rounded text-sm">
              {{ session('status') }}
            </div>
          @endif
